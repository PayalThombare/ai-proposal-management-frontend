...
const RecentProposalTable = ({ proposals }) => {
  ...
  if (proposals.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border p-6 text-center">
        <h2 className="text-xl font-bold text-gray-800 mb-3">Recent Proposals</h2>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-4">Recent Proposals</h2>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {proposals.slice(0, 5).map((proposal) => (
              <tr key={proposal._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
      ...
      {proposals.length > 5 && (
        <div className="mt-4 text-right">
          <Link to="/proposals" className="text-sm text-blue-600 hover:underline">
            View all proposals →
          </Link>
        </div>
      )}
